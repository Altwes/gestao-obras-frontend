...
import { HiLogout, HiPlus, HiRefresh, HiPencil, HiTrash } from 'react-icons/hi';
...
export default function DashboardPage() {
	...
	return (
		<main className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans relative">
			...
			<div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
					<div>
						...
						<p className="text-gray-600 dark:text-gray-400">Gerenciamento de orçamentos e medições</p>
					</div>
					<div className="flex gap-2">
						...
						<button onClick={() => { setObraParaEditar(null); setIsModalNovoOpen(true); }} className="flex items-center gap-2 px-6 py-3 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl shadow-lg transition-all">
						...
						<tbody className="divide-y divide-gray-200 dark:divide-gray-700">
							{loading ? (
								...
								<tr key={obra.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
									<td className="px-6 py-4 font-bold text-blue-700 dark:text-blue-400">{obra.numeroProtocolo}</td>
									<td className="px-6 py-4 text-gray-700 dark:text-gray-300">{obra.tipoOrcamento}</td>
									...
									<td className="px-6 py-4">
										{/* STATUS COM O PADRÃO DE CORES CORRETO */}
										<span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase inline-flex items-center gap-1 ${obra.status === 'ABERTO'
									...
									<td className="px-6 py-4 flex items-center justify-center gap-2">
										<button
											...
											className="px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-xs font-bold rounded-lg hover:bg-blue-600 hover:text-white transition-all"
										>
											Ver Itens
										</button>
										...
										{obra.status !== 'FINALIZADO' && (
											<>
												<button
													...
													className="px-3 py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-bold rounded-lg hover:bg-blue-700 hover:text-white transition-all"
												>
													Medir
												</button>
												<button
			...
			{/* MODAIS DE FEEDBACK E CONFIRMAÇÃO (UNIFICADOS) */}
			<ModalFeedback
			...
			<ModalConfirmacao
				...
				mensagem={confirmacao.msg}
				onConfirm={confirmacao.onConfirm}
				onClose={() => setConfirmacao({ ...confirmacao, open: false })}
			/>
			...
			/>
			<ModalMedicao
				isOpen={!!obraParaMedir}
				onClose={() => setObraParaMedir(null)}
				onSuccess={fetchOrcamentos}
				obra={obraParaMedir}
				showAlert={showAlert}
				showConfirm={showConfirm}
