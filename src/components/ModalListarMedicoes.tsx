...
export function ModalListarMedicoes({ isOpen, onClose, onSuccess }: Props) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 font-sans text-white">
        <div className="bg-[#1e293b] w-full max-w-6xl max-h-[92vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-gray-700 animate-in fade-in zoom-in duration-300">

          {/* Header Superior */}
          <div className="p-8 border-b border-gray-700 flex justify-between items-center bg-[#1e293b]">
            <div className="flex items-center gap-4">
              <div className="bg-blue-600/20 p-3 rounded-2xl">
                <HiClipboardList className="text-blue-500 w-8 h-8" />
              </div>
              <div>
                <h3 className="text-2xl font-black tracking-tight">Gerenciamento de Medições</h3>
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em]">Validação de Planilhas</span>
              </div>
            </div>
            <button onClick={onClose} className="p-3 hover:bg-red-500/20 text-gray-400 hover:text-red-500 rounded-full transition-all">
          ...
          <div className="p-8 overflow-y-auto flex-1">
            <div className="rounded-[2rem] border border-gray-700 bg-[#0f172a] overflow-hidden shadow-2xl">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[#1e293b] border-b border-gray-700">
                    <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest">Protocolo Vinculado</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase text-center">Nº Medição</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase text-center">Data</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase">Valor Medido</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase text-center">Status</th>
                    <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase text-center">Ações</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {medicoes.map((m) => (
                    <React.Fragment key={m.id}>
                      ...
                      >
                        <td className="px-8 py-6 font-bold text-blue-400">{m.orcamento?.numeroProtocolo}</td>
                        <td className="px-6 py-6 text-center font-black text-gray-400">#{m.numeroMedicao}</td>
                        <td className="px-6 py-6 text-center text-gray-500">{new Date(m.dataMedicao).toLocaleDateString()}</td>
                        <td className="px-6 py-6 font-black text-xl text-white">
                          {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(m.valorMedicao)}
                        </td>
                        <td className="px-6 py-6 text-center">
                          <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase flex items-center justify-center gap-2 ${m.status === 'VALIDADA' ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'
                            }`}>
                      ...
                      {/* DROP-DOWN DOS ITENS (SÓ SE ABERTA PODE EDITAR) */}
                      {expandedId === m.id && (
                        <tr className="bg-black/20">
                          <td colSpan={6} className="px-12 py-8 animate-in slide-in-from-top-4 duration-300">
                            <div className="bg-[#1e293b] rounded-[1.5rem] border border-gray-700 shadow-3xl overflow-hidden">
                              <table className="w-full text-xs">
                                <thead className="bg-[#0f172a] text-gray-500 font-black uppercase text-[8px] tracking-widest">
                                  <tr>
                                    <th className="px-6 py-4 text-left">Descrição do Serviço Executado</th>
                                    <th className="px-6 py-4 text-center">Quantidade Medida</th>
                                    <th className="px-6 py-4 text-right">Valor Parcial</th>
                                    {m.status === 'ABERTA' && <th className="px-6 py-4 text-center">Gestão de Itens</th>}
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-800">
                                      ...
                                      <td className="px-6 py-4 text-center">
                                        {itemSendoEditado === im.id ? (
                                          <input
                                            type="number"
                                            autoFocus
                                            className="w-24 p-2 text-center bg-[#0f172a] border border-blue-500/50 rounded-lg font-black text-blue-400 outline-none"
          ...
          {/* Footer de Ação */}
          <div className="p-8 border-t border-gray-700 bg-[#1e293b] flex justify-end">
            <button
              onClick={onClose}
              className="px-10 py-4 bg-gray-800 text-gray-400 font-black rounded-2xl hover:bg-gray-700 transition-all text-[10px] uppercase tracking-[0.2em] border border-gray-700 shadow-xl"
            >
      ...
      {/* COMPONENTES DE FEEDBACK MANTIDOS CONFORME SEU PADRÃO */}
      <ModalFeedback
      ...
      <ModalConfirmacao
        ...
        mensagem="Ao validar, os acumulados dos itens no orçamento serão atualizados permanentemente. Esta ação não pode ser desfeita e bloqueia edições futuras."
      />
    </>
  );
}